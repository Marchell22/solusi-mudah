...
class ExportBorrowings implements ShouldQueue
{
    ...
    public function __construct(array $columns = [], string $filename = null, $userId = null)
    {
        ...
        $this->filename = $filename ?? 'borrowings_' . date('Y-m-d_His') . '.xlsx';
        $this->userId = $userId;
    }
    ...
    public function handle(): void
    {
        ...
        Excel::store(
            new BorrowingsExport($this->columns),
        ...
        if ($this->userId) {
            ...
            if ($user) {
                ...
                Notification::send($user, new ExportCompleted('Borrowings', $url));
            }
        }
    }
}